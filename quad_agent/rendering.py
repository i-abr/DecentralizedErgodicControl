...
class Visual(object):
    ...
    def update_rendering(self, path):
        # assume we have the other classes inherited
        self._agent_marker.pose.position.x = self.state[0]*10.
        self._agent_marker.pose.position.y = self.state[1]*10.

        self._path_track.points = [
            Point(x[0]*10.,x[1]*10., self._agent_marker.pose.position.z) for x in path
        ]

        self._marker_pub.publish(self._agent_marker)
    ...
    def __build_rendering(self):
        ...
        self._agent_marker.pose.position.z = np.random.uniform(1,4)
