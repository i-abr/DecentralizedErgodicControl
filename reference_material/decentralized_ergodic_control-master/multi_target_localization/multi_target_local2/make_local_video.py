...
def update(i):
    ...
    for agent_no in range(no_agents):
        ...
        for target_no in range(no_targets):
            if _data is None:
                _data = agent_data[agent_no][target_no][i].ravel()
                # _data /= np.sum(_data)
            else:
                _temp = agent_data[agent_no][target_no][i].ravel()
                # _temp /= np.sum(_temp)
                _data += _temp
                # _data = np.amax( np.c_[_data.ravel(), _temp.ravel() ] ,axis=1)
        _data /= np.sum(_data)
        ax.contourf(X,Y, _data.reshape(X.shape), cmap="Greys")
        pc = PatchCollection(city_patches, facecolor='k', edgecolor='r')
        ax.add_collection(pc)
        for target_no in range(no_targets):
            ax.plot(target_data[target_no][0], target_data[target_no][1],'rx',lw=10)
        trajectory = agent_data[agent_no]['state']
        if i > 20:
            ax.plot(trajectory[i-20:i, 0], trajectory[i-20:i,1], lw=4)
        else:
            ax.plot(trajectory[0:i,0], trajectory[0:i,1], lw=4)

        ax.add_patch(Circle( (trajectory[i,0], trajectory[i,1]), 0.18, alpha=0.2 ))
...
if __name__ == '__main__':
    anim = FuncAnimation(fig, update, frames=data_len, interval=50)
    anim.save('test2.mp4', dpi=200)
