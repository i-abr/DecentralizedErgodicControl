...
class Phik(object):
    ...
    def __init__(self, settings):
        ...
        self.hiding_normfact,_ = monte_carlo(np.vectorize(self.__hiding_phi), self.xlim, n=200)
    ...
    def get_phik(self, phi): # use this for now
    ...
    def __phi(self, x, y, agent_state):
        value = exp(-0.5 * 40 * np.array(([x-agent_state[0],y-agent_state[1]])).dot([x-agent_state[0],y-agent_state[1]]))
    def __hiding_phi(self, x,y):
        value = 0.0
        for block in g_city_terrain.city_blocks:
            pose = block.position
            value = np.max( [value, exp(-40 * (x - pose[0])**2 )*exp(-40 *(y-pose[1])**2) ] )
        return value
    def hiding_phi(self, x, y):
        return self.__hiding_phi(x,y)/self.hiding_normfact

    def update_eid(self, agent_state):
        _pdf = lambda x, y: exp(-0.5 * 40 * np.array(([x-agent_state[0],y-agent_state[1]])).dot([x-agent_state[0],y-agent_state[1]]))
        ...
        # # phi = lambda x,y: _phi(x,y)/normfact
        phi = lambda x,y: np.amax([1.0-pdf(x,y),self.hiding_phi(x,y)])
        self.phik = self.get_phik(np.vectorize(phi))
        self.phik /= self.phik[0]
