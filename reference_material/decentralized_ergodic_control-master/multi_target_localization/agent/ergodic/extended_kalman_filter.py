...
class BearingOnlySensor(object):
    ...
    def h(self, xt, xs, ekf=False):
        ...
        if d <= self.range or ekf is True:
            if abs(xt[0]-xs[0]) < 0.001 or abs(xt[1] - xs[1]) < 0.001:
...
class EKF(object):
    ...
    def __init__(self, sys_dyn, sensor_model):
        ...
        self.mu = np.array([0.5] * self.sys_dyn.nX)
        ...
        self.__k = self.sys_dyn.nX
        self.__Z = 1.0/np.sqrt(self.det_sigma * (2.0 * pi)**self.__k)
        self.__has_sensed = True

    def pdf(self, _eval):
        if self.__has_sensed:
            delta = self.mu - _eval
            return self.__Z * np.exp(-0.5 * (delta).dot(self.inv_sigma).dot(delta))
        else:
            return 1.0
    ...
    def update(self, measurement, sensor_state):
        ...
        if np.linalg.norm(H) < 1e-5:
            ...
            if measurement is not None:
                ...
                mu = xk + K.dot(yk)
                # if np.linalg.norm(self.mu - mu) < 0.5:
                self.mu = mu
