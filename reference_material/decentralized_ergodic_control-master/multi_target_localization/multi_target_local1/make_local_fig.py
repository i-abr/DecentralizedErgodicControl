...
time_indx = [i for i in range(0, data_len,50)]
fig, ax = plt.subplots(no_agents, len(time_indx))
...
for i in range(len(time_indx)):
    for agent_no in range(no_agents):
        ...
        _data /= np.sum(_data)
        ax[agent_no,i].contourf(X,Y, _data.reshape(X.shape), cmap="Greys")
        pc = PatchCollection(city_patches, facecolor='k', edgecolor='r')
        ax[agent_no,i].add_collection(pc)
        for target_no in range(no_targets):
            ax[agent_no,i].plot(target_data[target_no][0], target_data[target_no][1],'x',lw=2)
        trajectory = agent_data[agent_no]['state']
        ax[agent_no,i].plot(trajectory[0:time_indx[i],0], trajectory[0:time_indx[i],1])
