...
class ErgodicObjective(object):
    ...
    def __init__(self, horizon, time_step):
        ...
        for i in range(self.coefs[0] + 1):
            for j in range(self.coefs[1] + 1):
                ...
                # self.lamk[i,j] = 1.0/(np.linalg.norm([i,j])**2+1)**(3.0/2.0)
    ...
    def l(self, x):
        ''' Running Cost '''
        return self.barrier.barr(x) + 200.0* self.floor_plan.get_penality(x)

    def ldx(self, x, ck, phik):
        dJerg = np.zeros(x.shape)
        dErg = (ck - phik).ravel()
        for i,k in enumerate(self.k_list):
            dJerg += self.lamk[i] * dErg[i] * self.basis.dfk(k, x) / 4.0
        t_erg = self.trajectory_distribution.get_remembered_time() * self.time_step
        return 2.0*self.q_ergodic*dJerg/(self.horizon+t_erg) + self.barrier.dbarr(x) + 200.0* self.floor_plan.first_order_derivative(x)
