...
targets = agent.objective.target_distribution.targets
...
data = [np.load('trial6/agent{}.npy'.format(i)) for i in range(num_agents)]
particle_data = [np.load('trial6/agent{}_particles.npy'.format(i)) for i in range(num_agents)]
...
height_scale = 0.2
target_locations = [[0.65426694, 0.69143373], [0.61644812, 0.45983947], [0.38885776, 0.64971471], [0.29730678, 0.77052033]]
...
def animate(i):
    ...
    # ax.plot(target_path[:i,0], target_path[:i,1], 'k', lw=2, zorder=10)

    for agent_no in range(num_agents):

        ax.scatter(particle_data[agent_no][i][:,0], particle_data[agent_no][i][:,1], alpha=0.5)
    ...
    for target_location in target_locations:
        target_artist = Circle((target_location[0], target_location[1]) , 0.02, color='k', zorder=10)
...
anim.save('trial6/target_localization.mp4', fps=30)
plt.show()
